...
#      Sort, define and store information here...
    for self.__newLine in self.__fileLine:
      ...
      if (self.__newLine.find('M2021-') != -1): self.storeFiber(self.__newLine)  ## dummy fiber
...
##
  def readLocalMeasurementFile(self):
    ...
    for self.__newLine in self.__inFile.readlines():
      if(self.__cmjDebug > 3): print(("XXXX readLocalMeasurmentFile : %s \n")% self.__newLine)
...
####  This done after the statistics for a batch have been loaded....
  def sendFiberVendorMeasurementToDatabase(self):
    if(self.__cmjDebug > 2):
      ...
      print(("XXXX __fiber__::sendFiberVendorMeasurementToDatabase: len(self.__vendorFiberId.keys()) = %d | len(self.__self.__vendorSpoolEnd.keys()) = %d ") % (len(list(self.__vendorFiberId.keys())),len(list(self.__vendorSpoolEnd.keys()))))
    self.__group = "Fiber Tables"
...
####      .......
  def sendFiberVendorAttenuationToDatabase(self):
    if(self.__cmjDebug > -1):
      ...
      print(("XXXX __fiber__::sendFiberVendorAttenuationToDatabase: len(self.__vendorFiberId.keys()) = %d | self.__vendorAttenuationTestDate = %s | len(self.__self.__vendorSpoolEnd.keys()) = %d | len(self.__vendorAttenuationDistance.keys) = %d  ") % (len(list(self.__vendorAttenuationFiberId.keys())),self.__vendorAttenuationTestDate,len(list(self.__vendorSpoolEnd.keys())),len(self.__vendorAttenuationDistance)))
    self.__group = "Fiber Tables"
...
##     that appears later in this script.
  def readLocalAttenuationFile(self):
    ...
    for self.__newLine in self.__inFile.readlines():
      if(self.__cmjDebug > 3): print(("XXXX readLocalAttenuationFile : %s \n")% self.__newLine)
...
##      Store the attenuation fiber id's:
  def storeLocalAttenuationFiberId(self,tempFiberId):
    ...
    for self.__newFiber in self.__item:
      self.__temporary = "xxxx"		## 2022Jan13
...
##
  def storeLocalAttenuationMeasurements(self,tempAttenLength):
    if(self.__cmjDebug > -1):
      print(("XXXX__fiber__:storeLocalAttenuationLocalMeasurements... tempWavelength = %s \n") % (tempAttenLength))
    self.__item = []
    ...
    for self.__mcount in range(len(self.__localAttenuationFiberId)):
      ...
      if(self.__cmjDebug > 6): print(("XXXX__fiber__:storeLocalAttenuationLocalMeasurements... self.__localAttenuationAdcCount[%s][%s] = %s ") % (self.__localAttenuationFiberId[self.__mcount],self.__item[1],self.__localAttenuationAdcCount[self.__localAttenuationFiberId[self.__mcount]][self.__localAttenuationLength[self.__item[1]]]))
    ...
    for self.__m in sorted(self.__localAttenuationId.keys()):
      ...
      ### Must load the fiber table first!
        ...
        if (self.__cmjDebug > 8): 
          ...
          print(("XXXX __fiber__::sendFiberLocalAttenuationMeasurementToDatabase: self.__localFiberId = %s") % (self.__localWavelengthTestId))
...
#### Diagnostic function to print out the fiber attenuation vs wavelength row:
  def dumpFiberLocalAttenuationMeasurementString(self):
      ...
      for self.__tempLocal in self.__sendLocalAttenuationMeasurementRow:
        print(('    self.__sendLocalAttenuationMeasurementRow[%s] = %s') % (self.__tempLocal,str(self.__sendWavelengthMeasurementRow[self.__tempLocal])))

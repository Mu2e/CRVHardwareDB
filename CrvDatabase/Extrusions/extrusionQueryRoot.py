...
##
    if(cmjPlotDiag != 0):		## diangonstic print statements
      ...
      for self.__m in self.__extrusionId.keys():
	print("...... self.__extrusionId.keys()     = %s || self.__extrusionId[%s] = %s \n")     % (self.__m,self.__m,self.__extrusionId[self.__m])
      ...
      for self.__m in self.__extrusionNumber.keys():
...
##   Plot scatter plots
  def plotScatterPlotsOld(self):
    ...
    for self.__event in self.__lightYield.keys():
...
##	Define the branches
    ...
    for self.__counter in self.__extrusionNumber.keys():

...
class multiWindow(Frame):
  def __init__(self,parent=NONE, myRow = 0, myCol = 0):
    self.__sleepTime = 0.2  ##  time interval between database requests
...
##  Add progress bar
    ...
    self.__progressbarStyle.theme_use('classic')
    #self.__progressbarStyle.configure("red.green.Horizontal.TProgressbar",troughcolor="red")
    self.__progressbarStyle.configure("green.Horizontal.TProgressbar",background="green")
    self.__progressbar = Progressbar(self.__frame0,style="green.Horizontal.TProgressbar",orient=HORIZONTAL,length=200,maximum=1000,variable=self.__progressBarCount,mode='determinate')
...
## -----------------------------------------------------------------
  def setStatusGrid2(self,tempSide):
    ...
    for nn in range(0,4):
      #
      self.__smbText = 'Layer '+str(nn)+' Smb'
    ...
    self.__title = 'CRV Module - '+self.__StringEntry_result+' --- '+self.__ModuleSideselection+' Side'
    ...
    for ppp in range(0,8):
      if(self.__localSide == 'top') : self.__tempPosition = self.__position_list_top[ppp]
      elif(self.__localSide == 'bottom') : self.__tempPosition = self.__position_list_bot[ppp]
      self.__positionBanner.append(myLabel(self.__moduleGridFrame,self.__tempRow,self.__tempColumn,int(4*self.__gridWidth)))
      self.__positionBanner[ppp].setText(self.__tempPosition)
    ...
    for mm in range(0,4):
      ...
      for nn in range(0,8):
       ...
       if(self.__localSide == 'top') : self.__cmb[mmm][nnn].setText(self.__moduleCmb_top_dict[mmm][nnn])
       elif(self.__localSide == 'bottom') : self.__cmb[mmm][nnn].setText(self.__moduleCmb_bot_dict[mmm][nnn])
       ...
       self.__cmb[mmm][nnn].grid(row=self.__tempRow,column=self.__tempColumn,columnspan=4)
       ##   --- smb ---
       self.__smb[mmm][nnn] = myLabel(self.__moduleGridFrame,self.__tempRow+1,self.__tempColumn,int(4*self.__gridWidth))
       if(self.__localSide == 'top') : self.__smb[mmm][nnn].setText(self.__moduleSmb_top_dict[mmm][nnn])
       elif(self.__localSide == 'bottom') : self.__smb[mmm][nnn].setText(self.__moduleSmb_bot_dict[mmm][nnn])
       self.__smb[mmm][nnn].setBorder(self.__borderWidth,self.__borderStyle)
       self.__smb[mmm][nnn].setBackgroundColor(self.__smbColor)
       self.__smb[mmm][nnn].grid(row=self.__tempRow+1,column=self.__tempColumn,columnspan=4)
       ##  --- diCounters ---
       ...
       for pp in range(0,4):
         ...
         if(self.__localSide == 'top') : self.__sipm_nest_dict[mmm][nnn][ppp].setText(self.__moduleSipmId_top_nest_dict[mmm][nnn][ppp])
         elif(self.__localSide == 'bottom') :self.__sipm_nest_dict[mmm][nnn][ppp].setText(self.__moduleSipmId_bot_nest_dict[mmm][nnn][ppp])
...
if __name__ == '__main__':
  ...
  root.geometry('1200x530+100+50')  ## set offset of primary window....

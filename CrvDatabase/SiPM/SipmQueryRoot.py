...
##  the selected batchs
  def countTheSimps(self):
    ...
    self.__SipmRows = []
    if(self.__cmjPlotDiag > 0) : print(("...multiWindow::countTheSipms: self.__database = %s self.__tables = %s \n") % (self.__database, self.__tables))
    self.__SipmRows = self.__getDatabaseValue.query(self.__database,self.__tables,"sipms_cnt") ## cmj 2022Jan28
    if(self.__cmjPlotDiag > 0) : print("...multiWindow::countTheSipms::Query Results:")
    for self.__SipmRow in self.__SipmRows:
      print((self.__SipmRow))
    if(self.__cmjPlotDiag > 1): print(("...multiWindow::countTheSipms:: number of rows = %s len(self.__SipmRows) = %d") % (self.__SipmRows,len(self.__SipmRows)))
...
##      Make querries to get all sipm batches to data base
  def getSipmsBatchesFromDatabase(self):
    if(self.__cmjPlotDiag > 0): print(".__multiWindow__getSipmBatchesFromDatabase: Enter \n") 
    ...
    if(self.__cmjPlotDiag > 1):
      for self.__l in self.__localSipmBatchValues:
        print(self.__l)
    if(self.__cmjPlotDiag > 0): print(".__multiWindow__getSipmBatchesFromDatabase: Exit \n")
...
##      Make querries to data base to get the sipm information
  def getSipmsFromDatabase(self,tempBatch):
    ...
    if(self.__cmjPlotDiag > 4) : 
      print ("__multiWindow__getSipmsFromDatabase ------------------------------------------------------------ ")
      print(("__multiWindow__getSipmsFromDatabase: List of Sipms with PO Number = %s \n") %(tempBatch))
      print(("__multiWindow__getSipmsFromDatabase: len(self.__singleSipmNumber) = %d ") % (len(self.__singleSipmNumber)))
      for self.__l in sorted(self.__singleSipmNumber):
        print(("__multiWindow__getSipmsFromDatabase: self.__l  = %s ") % (self.__l))
    ...
    for self.__localSipmNumber in self.__singleSipmNumber:
      #xx = tk.DoubleVar()
      #xx = self.__progressBarCount.get()
      #self.__progressBarCount(xx+10)
      if(self.__localSipmNumber != "NULL" and self.__localSipmNumber != ''):
        print(("__multiWindow__getSipmsFromDatabase: get sipm_id  = %s") % (self.__localSipmNumber))
        self.__localSipmResult = []
        self.__fetchCondition = "sipm_id:eq:"+str(self.__localSipmNumber)
        if (self.__cmjPlotDiag > 4):
          print(("__multiWindow__getSipmsFromDatabase: self.__table  = %s \n") % (self.__table))
          print(("__multiWindow__getSipmsFromDatabase: self.__fetchThese  = %s \n") % (self.__fetchThese))
          print(("__multiWindow__getSipmsFromDatabase: self.__fetchCondition = %s \n") % (self.__fetchCondition))
        ...
        if(self.__cmjPlotDiag > 4): print(("__multiWindow__getSipmsFromDatabase: len(self.__localSipmResult) = %d, self.__localSipmResult  = %s \n") % (len(self.__localSipmResult),self.__localSipmResult))
        self.__mcount = 0
        self.__ncount = 0
        for self.__mline in sorted(self.__localSipmResult):
          ...
          self.__progressBarCount.set(xx+10)  ## increment that value
          self.update() ## update the progress bar...
          if(self.__cmjPlotDiag > 5): print(("__multiWindow__getSipmsFromDatabase: (self.__singleSipmLine, all) ncount = %d,  len(self.__mline) = %d,  self.__mline  = %s \n") % (self.__ncount,len(self.__mline),self.__mline))
          if(len(self.__mline) > 4):
            self.__singleSipmLine.append(self.__mline.rsplit(','))
            if(self.__cmjPlotDiag > 2): print(("__multiWindow__getSipmsFromDatabase: (self.__singleSipmLine, filtered) mcount = %d, self.__mline  = %s \n") % (self.__mcount,self.__mline))
...
##
  def fillHistograms(self):
    ...
    for self.__testDate in list(self.__sipmMeasureTestDate.keys()):
      for self.__event in list(self.__sipmId[self.__testDate].keys()):
...
##  Use Root...
  def defineAllMeasurementRootTree(self):
    ...
    self.__vect_SipmId = _root.vector('string')()  ## cmj2022Jan25
    self.__vect_SipmTestDate = _root.vector('string')()  ## cmj2022Jan25
    self.__vect_SipmTestType = _root.vector('string')()  ## cmj2022Jan25
    self.__vect_WorkerBarCode = _root.vector('string')()  ## cmj2022Jan25
    self.__vect_WorkStationBarCode = _root.vector('string')()  ## cmj2022Jan25
    self.__vect_DataFileLocation = _root.vector('string')()  ## cmj2022Jan25
    self.__vect_DataFileName = _root.vector('string')()  ## cmj2022Jan25
    self.__vect_PackNumber = _root.vector('string')()  ## cmj2022Jan25
...
##      First define a folder with the test date
    ...
    self.__allRootTree.Branch('numberOfEntries',self.__arrayAllNumberOfEntries,'AllNumberOfEntries[1]/I')  ## cmj2022Jan25 
    self.__allRootTree.Branch('sipmId',self.__vect_SipmId)  ## cmj2022Jan25 
    self.__allRootTree.Branch('sipmTestDate',self.__vect_SipmTestDate)  ## cmj2022Jan25 
    self.__allRootTree.Branch('sipmTestType',self.__vect_SipmTestType)  ## cmj2022Jan25 
    #
    self.__allRootTree.Branch('workerBarCode',self.__vect_WorkerBarCode)  ## cmj2022Jan25 
    self.__allRootTree.Branch('workstationBarCode',self.__vect_WorkStationBarCode)  ## cmj2022Jan25 
    self.__allRootTree.Branch('dataFileLocation',self.__vect_DataFileLocation)  ## cmj2022Jan25 
    self.__allRootTree.Branch('dataFileName',self.__vect_DataFileName)  ## cmj2022Jan25 
    self.__allRootTree.Branch('packNumber',self.__vect_PackNumber)  ## cmj2022Jan25 
    #
    ...
    for self.__m in list(self.__allSipmId.keys()) :            ## load the arrays to be plotted.
      ...
      ## cmj2022Jan25... add std::vector<std::string> to the root trees
      self.__vect_SipmId.push_back(self.__allSipmId[self.__m])
      self.__vect_SipmTestDate.push_back(self.__allSipmMeasureTestDate[self.__m])
      self.__vect_SipmTestType.push_back(self.__allTestType[self.__m])
      self.__vect_WorkerBarCode.push_back(self.__allWorkerBarCode[self.__m])
      self.__vect_WorkStationBarCode.push_back(self.__allWorkStationBarCode[self.__m])
      self.__vect_DataFileLocation.push_back(self.__allData_file_location[self.__m])
      self.__vect_DataFileName.push_back(self.__allData_file_name[self.__m])
      self.__vect_PackNumber.push_back(self.__allPack_number[self.__m])
      ...
      if(self.__cmjPlotDiag > 0):
        print(("__multiWindow__defineAllSipmMeasurementRootTree:: Dump Root Tree Record"))
        print(".................................")
	#print((" self.__allPlotSipmId[%s] = %s ")  % (self.__m, self.__allPlotSipmId(self.__m)))
        print((" self.__allBiasVoltage[%s] = %f")  % (self.__m, self.__allBiasVoltage[self.__m]))
        print((" self.__allDarkCount[%s] = %f")  % (self.__m,self.__allDark_count_rate[self.__m]))
        print((" self.__allGain[%s] = %f")  % (self.__m,self.__allGain[self.__m]))
        print((" self.__allTemperature[%s] = %f")  % (self.__m, self.__allTemperature[self.__m]))
        print((" self.__allBreakdown_voltage[%s] = %f")  % (self.__m, self.__allBreakdown_voltage[self.__m]))
        print((" self.__allDark_count_rate[%s] = %f")  % (self.__m,self.__allDark_count_rate[self.__m]))
        print((" self.__allCurrent_vs_voltage_condition[%s] = %f")  % (self.__m,self.__allCurrent_vs_voltage_condition[self.__m]))
        print((" self.__allX_talk[%s] = %f")  % (self.__m,self.__allX_talk[self.__m]))
        print((" self.__allLed_response[%s] = %f")  % (self.__m,self.__allLed_response[self.__m]))
        print((" self.__allSipmId[%s] = %s")  % (self.__m,self.__allSipmId[self.__m]))
        print((" self.__allTestType[%s] = %s")  % (self.__m,self.__allTestType[self.__m]))
        print((" self.__allWorkerBarCode[%s] = %s")  % (self.__m,self.__allWorkerBarCode[self.__m]))
        print((" self.__allWorkStationBaCode[%s] = %s")  % (self.__m,self.__allWorkStationBarCode[self.__m]))
        print((" self.__allData_file_location[%s] = %s")  % (self.__m,self.__allData_file_location[self.__m]))
        print((" self.__allData_file_name[%s] = %s")  % (self.__m,self.__allData_file_name[self.__m]))
        print((" self.__allPack_number[%s] = %s")  % (self.__m,self.__allPack_number[self.__m]))
        print(".................................")
...
##  Define a Root TTree for each SiPM I vs V measurement...
  def defineSipmIvsVMeasurementRootTree(self):
    if(self.__cmjPlotDiag > 0): print("XXXXXXXXXXX __multiWindow__defineSipmIvsVMeasurementRootTree:: Enter \n")
    ...
    if(self.__cmjPlotDiag > 0): print("XXXXXXXXXXX __multiWindow__defineSipmIvsVMeasurementRootTree:: Exit \n")
    return
##
##################################################################################################
##################################################################################################
...
##  Use Root...
  def definePyRootTree(self):
    if(self.__cmjPlotDiag > 0): print("XXXXXXXXXXX__multiWindow__definePyRootTree::  Enter definePyRootTree \n")
...
##      Define the branches
    ...
    self.__rootTreeFile.Close()
    if(self.__cmjPlotDiag > 0): print("XXXXXXXXXXX__multiWindow__definePyRootTree::  Exit definePyRootTree \n")
    return  
##
## --------------------------------------------------------------------
##

# -*- coding: utf-8 -*-
##  File = "sipmRecievedBatches2016Jun24.py"
##  Derived from File = "sipmRecievedBatches2016Jan12.py"
##  Derived from File = "sipmRecievedBatches2016Jan6.py"
##
...
##  Modified by cmj2019Jul13... Use cmjGuiLibGrid2019Jan30
...
from cmjGuiLibGrid import *  ## cmj 2020Aug03 
...
## class packSipmBatchWindow(Frame):
...
####  First Column Entries
	  ...
	  self.__mySaveIt.setOutputFileName('sipm_orders_recieved')
...
#                   Enter Quantity Received... 
	  self.__SipmQuantityReceivedInt = myIntEntry(self,self.__row,self.__col,self.__mySaveIt)
	  self.__strName.append("SiPM Quantity Received")
	  self.__SipmQuantityReceivedInt.setEntryText(self.__strName[self.__sCount])
	  self.__sCount += 1
	  self.__SipmQuantityReceivedInt.setLabelWidth(self.__labelWidth)
	  self.__SipmQuantityReceivedInt.setEntryWidth(self.__entryWidth)
	  self.__SipmQuantityReceivedInt.setButtonWidth(self.__buttonWidth)
	  self.__SipmQuantityReceivedInt.makeEntry()
	  self.__SipmQuantityReceivedInt.grid(row=self.__row,column=self.__col,stick=W)
	  self.__row += 1
#                   Enter Date Received... 
	  self.__SipmReceivedDateStr = myStringEntry(self,self.__row,self.__col,self.__mySaveIt)
	  self.__strName.append("SiPM Received Date (mm/dd/yyyy)")
	  self.__SipmReceivedDateStr.setEntryText(self.__strName[self.__sCount])
	  self.__sCount += 1
	  self.__SipmReceivedDateStr.setLabelWidth(self.__labelWidth)
	  self.__SipmReceivedDateStr.setEntryWidth(self.__entryWidth)
	  self.__SipmReceivedDateStr.setButtonWidth(self.__buttonWidth)
	  self.__SipmReceivedDateStr.makeEntry()
	  self.__SipmReceivedDateStr.grid(row=self.__row,column=self.__col,stick=W)
	  self.__row += 1
#### Second Column entries
	  ...
	  self.__myInstructions.setText('','Instructions/InstructionsForReceivedPurchaseOrder2015Jul30.txt')
...
#         Display the date the script is being run
          ...
          self.__row += 1
##	Add button to retrieve the last few Batches entered into the database
##	This appears in a separate window with a scroll bar
	  ...
	  self.__button1.setAction('mu2e_SiPMReceivedPurchaseOrders_2016Jun24')
...
##    Add the "Enter Data Base" button
          ...
          self.__next.grid(row=self.__maxRow,column=1,sticky=E)
##              Add status bar below the control bar
    ...
    ## This method calls the method to write the GUI entries to the database
	  ...
	  self.__sipmPurchaseReceivedString = self.buildRowString_for_SiPM_Purchase_Received_table()
	  if sendDataBase != 0:
	    print "send to database!"
	    myDataLoader1 = DataLoader(self.__password,self.__url,self.__group,self.__table)
	    myDataLoader1.addRow( self.__sipmPurchaseReceivedString)
	  ...
	  self.__SipmQuantityReceivedInt.resetEntry()
	  self.__SipmReceivedDateStr.resetEntry()

    ##-----------------------------------------------------------------
    ## This method retrieves the entries from the GUI fields and builds
    ## a dictionary for to pass on to the database... the SiPM_batches table...
      def buildRowString_for_SiPM_Purchase_Received_table(self):
	  ...
	  self.__sendRow['quantity_received'] = self.__mySaveIt.getIntEntry('SiPM Quantity Received')
	  self.__sendRow['date_received'] = self.__mySaveIt.getStrEntry('SiPM Received Date (mm/dd/yyyy)')
...
if __name__ == '__main__':
     root = Tk()              # or Toplevel()
     bannerText = 'Mu2e:: SiPM Received Batches'
     root.title(bannerText)  
#     myWindow=packSipmBatchWindow(root,0,0).grid()
